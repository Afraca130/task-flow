...
export class TasksService {
  ...
  async findByProjectIdAndStatus(
    ...
    return await this.taskRepository.findByProjectIdAndStatusOrderedByRank(
      projectId,
      status,
    );
  }
  ...
  async createTask(command: CreateTaskDto): Promise<Task> {
    ...
    try {
      ...
      const existingTasks =
        await this.taskRepository.findByProjectIdAndStatusOrderedByRank(
          command.projectId,
          taskStatus,
        );
      ...
      task.dueDate = command.dueDate
        ? TimeUtil.toDate(command.dueDate)
        : undefined;
      ...
      if (command.assigneeId) {
        try {
          this.logger.log(
            `🔔 Preparing to send task assignment notification to: ${command.assigneeId}`,
          );
          ...
          this.logger.log(
            `👤 Assigner details: ${assignerName} (${command.assignerId})`,
          );
          ...
          this.logger.log(
            `✅ Task assignment notification created: ${notification.id}`,
          );
  ...
  async updateTask(
    ...
    try {
      ...
      if (
        ...
        command.description !== existingTask.description
      ) {
        changes.description = {
      ...
      if (
        ...
        command.status !== existingTask.status
      ) {
      ...
      if (
        ...
        command.priority !== existingTask.priority
      ) {
        changes.priority = {
      ...
      if (
        ...
        command.assigneeId !== existingTask.assigneeId
      ) {
        changes.assignee = {
          from: existingTask.assigneeId,
          to: command.assigneeId,
        };
  ...
  async reorderTask(
    ...
    try {
      ...
      const allTasksInStatus =
        await this.taskRepository.findByProjectIdAndStatusOrderedByRank(
          projectId,
          targetStatus,
        );

      const duplicates = allTasksInStatus.filter(
        task => task.lexoRank === newLexoRank && task.id !== taskId,
      );

      if (duplicates.length > 0) {
        this.logger.warn(
          `Found ${duplicates.length} tasks with duplicate LexoRank`,
        );
          ...
          updatedTask.title,
          {
            reordered: {
              ...
              statusChanged: statusChanged
                ? { from: taskToMove.status, to: targetStatus }
                : null,
            },
          },
        );
  ...
  async reorderTaskByLexoRank(
    ...
    this.logger.log(
      `Reordering task: ${taskId} with new LexoRank: ${newLexoRank}`,
    );
  ...
  async getTaskStatsByProject(projectId: string): Promise<{
    ...
    try {
      ...
      this.logger.log(
        `Task statistics retrieved for project ${projectId}: ${JSON.stringify(stats)}`,
      );
      return stats;
    } catch (error) {
      this.logger.error(
        `Failed to get task statistics for project: ${projectId}`,
        error.stack || error,
      );
      throw error;
    }
