...
export class InvitationsService {
  ...
  async createInvitation(
    ...
    try {
      ...
      this.logger.log(
        `🏭 Created invitation entity with token: ${invitation.token}`,
      );
      ...
      this.logger.log(
        `✅ Invitation created successfully: ${savedInvitation.id}`,
      );
      this.logger.log(
        `🔗 Invitation token stored: ${savedInvitation.token || 'NO TOKEN FOUND!'}`,
      );

      // Create notification for the invitee
      try {
        this.logger.log(
          `🔔 Preparing invitation notification for user: ${createDto.inviteeId}`,
        );
        ...
        this.logger.log(
          `📁 Project details: ${projectName} (${createDto.projectId})`,
        );
        ...
        this.logger.log(
          `✅ Invitation notification created: ${notification.id}`,
        );
      ...
      this.logger.error(
        `Failed to create invitation for project: ${createDto.projectId}`,
        error,
      );
      throw error;
    }
  ...
  async acceptInvitation(
    ...
    this.logger.log(
      `🎯 Accepting invitation with token: ${token} by user: ${userId}`,
    );
    ...
    try {
      ...
      if (invitation.status !== InvitationStatus.PENDING) {
        this.logger.warn(
          `⚠️ Invitation already processed: ${invitation.status}`,
        );

        if (invitation.status === InvitationStatus.ACCEPTED) {
          // If already accepted, just return success (idempotent)
          this.logger.log(
            `✅ Invitation already accepted, returning existing result`,
          );
          return invitation;
          ...
          throw new BadRequestException(
            `This invitation has status: ${invitation.status}`,
          );
        }
      }
      ...
      try {
        this.logger.log(
          `👥 Adding user ${userId} to project ${invitation.projectId}`,
        );
      ...
      this.logger.log(
        `🎉 Invitation accepted successfully: ${updatedInvitation.id}`,
      );
  ...
  async declineInvitation(
    ...
    try {
      ...
      this.logger.log(
        `Invitation declined successfully: ${updatedInvitation.id}`,
      );
      return updatedInvitation;
    } catch (error) {
      this.logger.error(
        `Failed to decline invitation with token: ${token}`,
        error,
      );
      throw error;
    }
