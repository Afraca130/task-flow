...
export class NotificationsService {
  ...
  async createNotification(
    ...
    try {
      ...
      this.logger.log(
        `Notification created successfully: ${savedNotification.id}`,
      );

      return savedNotification;
    } catch (error) {
      this.logger.error(
        `Failed to create notification for user: ${createDto.userId}`,
        error,
      );
      throw error;
    }
   ...
   * Skip notification if assignee is the same as assigner
   */
  ...
  async createIssueMentionNotifications(
    ...
    for (const userId of mentionedUserIds) {
      try {
        ...
        this.logger.error(
          `Failed to create mention notification for user ${userId}:`,
          error,
        );
      }
    }
